...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      if (mounted && _controller != null && _controller!.value.isInitialized) {
        ...
        print(
            "HomeScreen: No se inició el image stream (widget desmontado o controlador no inicializado).");
      }
    } catch (e) {
  ...
  void _onCameraImage(CameraImage image) {
    ...
    () async {
      try {
        ...
        final input =
            inputImageFromCameraImage(image, _controller!.description);
        ...
        if (faces.isNotEmpty) {
          ...
          if (angleY == null ||
            ...
            final img.Image cropped =
                img.copyCrop(rgb, x: x, y: y, width: w, height: h);
  ...
  Future<void> _registerAttendance({required bool isIngress}) async {
    ...
    if (id == null && _lastEmbedding != null) {
      try {
        print(
            'HomeScreen: _registerAttendance - Re-intentando identificar con último embedding...');
        final match =
            await ServiceLocator.recognition.identify(_lastEmbedding!);
        if (match != null) {
          id = match.id;
          _lastDetectedId = match.id;
          _lastDetectedName = match.name;
          ...
          if (mounted) setState(() {});
          print(
              'HomeScreen: _registerAttendance - Identificación exitosa en re-intento.');
        } else {
          print('HomeScreen: _registerAttendance - Re-intento fallido.');
        }
      } catch (e) {
        print(
            'HomeScreen: _registerAttendance - Error en re-intento de identificación: $e');
      }
    }
    if (id == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('No se detectó identidad válida recientemente')),
        );
      }
      return;
    }
    try {
      ...
      if (mounted) {
        ...
        final String suffix =
            _lastDetectedDocument != null ? ' · Doc: $_lastDetectedDocument' : '';
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                '${eventType.replaceFirst('e', 'E')} registrada para $label$suffix')));
      ...
      body: FutureBuilder<void>(
        ...
        builder: (context, snapshot) {
          ...
          if (ctrl == null || !ctrl.value.isInitialized) {
            print(
                "HomeScreen: Build - Inicialización completada, pero cámara no lista (ctrl is null: ${ctrl == null}, isInitialized: ${ctrl?.value.isInitialized})");
            return const Center(
                child: Text(
                    'Cámara no disponible. Verifique permisos o reinicie.'));
          }
                ...
                child: IgnorePointer(
                  child: Container(
                    decoration: BoxDecoration(
                      border: Border.all(
                          color: kPrimaryColor.withOpacity(0.5), width: 6),
                    ),
                  ),
                ),
              ),
